...
const lexicon = {
	...
	technologies: ['3D Printer', '3D Printing', '3D Scanner', '3D Scanning', 'CAN bus', 'Microcontrollers', 'PCBs'],
	software: [
		'API Gateway',
		'AWS Lambda',
		'CATIA',
...
document.addEventListener('DOMContentLoaded', () => {
	...
	function cardMissingBackgroundImage(card) {
		...
		// console.log("card doesn't have background image");
		return true;
	}
	...
	function fetchPositionsAndAssignProjects(positionTitle) {
		...
		getNonActiveCards().forEach((card, index) => {
			if (projects[index]) {
				...
				// if (projectNotMissingBackgroundImage(project.name)) {
		...
		data.positions.forEach((position) => {
			if (position.position === getPositionFrom(card)) {
				...
				role_tags.forEach((skill, index) => {
					...
					setTimeout(() => {
						bottom.querySelector('.role_tags').appendChild(tag);
					}, index * tag_animation_delay); // 100ms delay between each tag
				});
				...
				achievement_tags.forEach((skill, index) => {
					...
					tag.textContent = skill;
					setTimeout(() => {
						if (tag) bottom.querySelector('.achievement_tags').appendChild(tag);
					}, index * tag_animation_delay); // 100ms delay between each tag
				});
